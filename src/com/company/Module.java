...
public class Module {
    //用户的登陆 根据ID登陆 进行数据比对
    //用户的注册  新添加记录。
    public static Person login(String ID)throws Exception{
        ...
        if(resultSet.next()){//查到了数据 就将数据库的数据返回给Person类
            Person person=new Person();
            person.SetID(resultSet.getString("user_id"));
